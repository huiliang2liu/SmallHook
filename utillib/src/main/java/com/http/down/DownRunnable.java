...
class DownRunnable implements Runnable {
    ...
    public DownRunnable(String url, int threads, Context context, ExecutorService service, File file, DownListener listener) {
        ...
        if (!parent.exists())
            parent.mkdirs();
        savePath = new File(context.getExternalFilesDir("down"), MD5.String2MD5Method161(url) + ".xml");
        parent = file.getParentFile();
        if (!parent.exists())
            parent.mkdirs();
        temporaryFile = new File(parent, MD5.String2MD5Method161(url));
        mUrl = url;
        mThreads = threads;
        mFile = file;
    ...
    private  void save() {
        ...
        LogUtil.e(TAG, "保存数据" + json);
        if (savePath.exists())
            savePath.delete();
        StreamUtil.stream2file(StreamUtil.string2stream(json), savePath);
    }
    ...
    private List<DownEntity> url2entities(String url, int threads) {
        ...
        try {
            if (savePath.exists()) {
                String s = StreamUtil.stream2string(new FileInputStream(savePath));
                if (s != null && !s.isEmpty()) {
