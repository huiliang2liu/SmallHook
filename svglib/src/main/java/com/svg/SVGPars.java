 ...
 **/
public class SVGPars extends ASVGPars {
    private static SVGPars parser;


    public static SVGPars init() {
        if (parser == null) {
            synchronized ("SVGPars") {
                if (parser == null)
                    parser = new SVGPars();
            }
        }
    ...
    public static NumberParse parseNumbers(String s) {
        // PathUtil.debug("Parsing numbers from: '" + s + "'");
        int n = s.length();
        int p = 0;
